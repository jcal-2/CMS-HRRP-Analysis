...
# dbt output dataset (where fct_ and dim_ tables live)
# Update this to match your dbt target schema
DBT_DATASET = "cms_penalty_analysis"

# Raw historical dataset (FY-specific HRRP tables)
...
def query_penalty_panel():
    ...
    Adapt the SQL below to match your dbt model names. If you have a
    fct_hospital_penalties table that unions all 10 FYs, use that directly.
    Otherwise this queries the raw FY-specific tables.
    """
    
    # Option A: Query from a dbt fact table (preferred if it exists)
    # Uncomment and adapt if you have a union table:
    #
    # sql = f"""
    # SELECT 
    #     facility_id,
    #     fiscal_year,
    #     penalty_pct,
    #     err_ami, err_hf, err_pn, err_copd, err_tka, err_cabg,
    #     measures_above_expected,
    #     peer_group
    # FROM `{PROJECT_ID}.{DBT_DATASET}.fct_hospital_penalties`
    # ORDER BY facility_id, fiscal_year
    # """
    
    # Option B: Union from raw FY-specific tables
    # Adapt column names to match your ingested schema
    fy_tables = []
    for fy in range(2016, 2026):
        ...
        SELECT 
            facility_id,
            {fy} as fiscal_year,
            SAFE_CAST(penalty_pct AS FLOAT64) as penalty_pct,
            SAFE_CAST(err_ami AS FLOAT64) as err_ami,
            SAFE_CAST(err_hf AS FLOAT64) as err_hf,
            SAFE_CAST(err_pn AS FLOAT64) as err_pn,
            SAFE_CAST(err_copd AS FLOAT64) as err_copd,
            SAFE_CAST(err_tka AS FLOAT64) as err_tka,
            SAFE_CAST(err_cabg AS FLOAT64) as err_cabg
        FROM `{PROJECT_ID}.{RAW_DATASET}.hrrp_fy{fy}`
        """)
    
    sql = " UNION ALL ".join(fy_tables) + " ORDER BY facility_id, fiscal_year"
...
def query_hospital_characteristics():
    ...
    SELECT 
        ...
        hac_score,
        vbp_tps
    FROM `{PROJECT_ID}.{DBT_DATASET}.fct_hospital_current_performance`
    """
